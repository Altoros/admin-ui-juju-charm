    ...
    def remote_units(cls):
        ...
        for rid in sorted(hookenv.relation_ids(cls.name)):
            for unit in sorted(hookenv.related_units(rid)):
                units.add(hookenv.relation_get('private-address',
                                               rid=rid, unit=unit))
        return list(units)
...
class CloudFoundryCredentials(RelationContext):
    """Provide access to CF level administration credentials.
    Attaching this relation to a remote service gives them the ability to
    manipulate the CF deployment as an administrator through the CLI or
    ...
    def get_domain(self):
        ...
        if domain == 'xip.io':
            ...
            if 'haproxy' in status['services']:
                ...
                domain = "%s.xip.io" % utils.to_ip(public_address)
        return domain

    def get_artifacts_url(self):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not any([self.ne, self.eq]):
            with open(os.path.join(hookenv.charm_dir(), 'config.yaml')) as fp:
                self.ne = {o: v.get('default') for o, v in yaml.load(
                    fp).get('options', {}).items()}
        else:
            self.required_options = list(
                set(self.required_options + self.ne.keys() + self.eq.keys()))
