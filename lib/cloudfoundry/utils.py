...
def setup_modprobe(module):
    ...
    try:
        ...
        # In certain containers we won't have access to kernel modules
        # continue w/o
        pass
...
def jsonify(obj):
    return json.dumps(obj, indent=2, default=serializable, sort_keys=True)


def to_ip(address):
    """Map a string address to a publicly reachable IP address."""
    if ip_pat.match(address):
        return address  # already an IP
    else:
        return resolve_public_ip(address)


def resolve_public_ip(address):
    """Attempt to resolve the addresses from google public DNS followed by
    machine local DNS."""
    def filter_ip(dig_output):
        for candidate in dig_output.split('\n'):
